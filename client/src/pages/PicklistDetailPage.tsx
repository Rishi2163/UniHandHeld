...
export const PicklistDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPendingShelves = async () => {
      try {
        const response = await fetchShelvesInPicklist(picklistId);
        if (response) {
  ...
  const handleBack = () => setLocation("/b2b-packing");
  const handleStartPicking = () => {
    if (isBarcodeMode) setLocation(`/tote-scanner/${picklistId}`);
    else setLocation(`/shelf-selection/${picklistId}`);
  };

  const handleMoreOptions = () => console.log("More options clicked");
  const handleSortToggle = () => setSortOrder((prev) => (prev === "asc" ? "desc" : "asc"));
  const handleSectionClick = () => setIsSectionDialogOpen(true);
  const handleSectionSelect = (sectionName: string) => {
    setSelectedSection(sectionName);
    setIsSectionDialogOpen(false);
  };
  const handleMenuClick = () => setIsSideNavOpen(true);
  ...
  return (
    <div className="bg-white min-h-screen w-full">
      ...
      <div className="bg-white w-full max-w-md mx-auto min-h-screen relative">
        <header className="flex h-12 items-center justify-between w-full bg-white border-b border-[#e0e0e0] px-4">
          <div className="flex items-center gap-3">
            <button className="w-8 h-8 hover:bg-gray-100 rounded" onClick={handleMenuClick}>
              <MenuIcon className="h-5 w-5 text-text-elementsprimary" />
            </button>
            <button className="w-8 h-8 hover:bg-gray-100 rounded" onClick={handleBack}>
              <ArrowLeft className="h-5 w-5 text-text-elementsprimary" />
            </button>
            <h1 className="font-PAGE-TITLE text-text-elementsprimary">{picklistId}</h1>
          </div>
          <button className="w-8 h-8 hover:bg-gray-100 rounded" onClick={handleMoreOptions}>
            <MoreVertical className="h-5 w-5 text-text-elementsprimary" />
          </button>
        </header>
        ...
        <div className="flex flex-col pt-4 px-4 pb-20">
          ...
          <div className="flex items-center justify-between mb-4 px-2">
            <button onClick={handleSectionClick} className="text-sm font-medium text-gray-600 uppercase">
              SECTION
            </button>
            <button onClick={handleSortToggle} className="flex items-center gap-1 px-2 py-1 rounded">
              <span className="text-sm text-gray-500">Shelf Code</span>
            ...
            {activeTab === "pending" ? (
              sortedShelves.map((shelf) => (
                <Card key={shelf.id} className="border border-gray-200 rounded-lg shadow-sm">
                  <CardContent className="p-4">
                    <h3 className="text-lg font-semibold text-gray-900 mb-3">{shelf.shelfCode}</h3>
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col">
                        <span className="text-sm text-gray-500">SKU Count</span>
        ...
        <Dialog open={isSectionDialogOpen} onOpenChange={setIsSectionDialogOpen}>
          <DialogContent className="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-[412px] max-w-[90vw] rounded-t-2xl border-0 p-0 bg-white shadow-xl">
            <DialogHeader className="sr-only">
              <DialogTitle>Select Section</DialogTitle>
            ...
            <div className="flex justify-between items-center p-4 border-b border-gray-200">
              <DialogClose asChild>
                <Button variant="ghost" className="text-blue-600 font-medium p-0 h-auto">
                  CANCEL
                </Button>
              </DialogClose>
              <Button
            ...
            <div className="flex flex-col py-4">
              {sections.map((section) => (
                <button
                  key={section.id}
                  className={`text-center py-4 text-lg border-b ${
                    selectedSection === section.name
                      ? "text-blue-600 font-medium bg-blue-50"
                      : "text-gray-800 hover:text-blue-600 hover:bg-gray-50"
                  }`}
