...
export const ShelfDetailPage: React.FC = () => {
  ...
  const handleShelfClick = (shelfCode: string,shelfStatus:boolean) => {
      if (!shelfStatus) {
    setClearedShelves((prev) => [...prev, shelfCode]);
  }
    ...
    <div className="bg-white min-h-screen w-full">
      <div className="bg-white w-full max-w-md mx-auto min-h-screen relative">
        ...
        <div className="flex-1 overflow-y-auto">
          {sortedShelves.filter((shelf) => !clearedShelves.includes(shelf.code))
  .map((shelf) => (
            <button
              key={shelf.code}
