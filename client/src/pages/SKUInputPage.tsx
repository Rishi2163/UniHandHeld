...
//       try {
//         const data = await fetchSKUItemsFromShelf(id, shelfCode);
//         setProducts(data);
//       } catch (error) {
//         console.error('Failed to fetch SKU items:', error);
//       }
//     };
//     fetchData();
//   }, [id, shelfCode]);
...
//       const updatedProducts = products.map(product => {
//         if (product.sku.includes(skuInput.trim()) && product.pickedQuantity < product.quantity) {
//           return { ...product, pickedQuantity: product.pickedQuantity + 1 };
//         }
//         return product;
//       });
//       setProducts(updatedProducts);
...
//     return `data:image/svg+xml,${encodeURIComponent(`
//       <svg viewBox="0 0 80 40" xmlns="http://www.w3.org/2000/svg">
//         <path d="M10 25 Q15 15 35 18 Q55 20 70 25 Q65 35 45 32 Q25 30 10 25 Z" fill="${colorMap[color] || '#6b7280'}" stroke="#374151" stroke-width="1"/>
//         <path d="M15 22 Q25 20 45 22 Q55 23 65 25" stroke="white" stroke-width="1.5" fill="none"/>
//         <circle cx="20" cy="38" r="1.5" fill="white" opacity="0.6"/>
//         <text x="32" y="18" text-anchor="middle" fill="#64748b" font-size="6" font-family="Arial, sans-serif" font-weight="bold">NIKE</text>
//       </svg>
...
//         <div className="flex gap-3">
//           <div className="w-16 h-16 bg-gray-100 rounded flex-shrink-0 flex items-center justify-center">
//             <img src={generateShoeImage(product.color)} alt="Product" className="w-full h-full object-cover" />
//           </div>
//           <div className="flex-1">
...
//                 </span>
//                 {!isPending && (
//                   <button onClick={() => handleUnpickItem(product.id)} className="text-blue-600 ml-1 text-xs hover:underline">
//                     (click to unpick)
//                   </button>
//                 )}
//               </div>
...
//           <button
//             className={`flex-1 py-3 text-sm font-medium border-b-2 transition-colors ${
//               activeTab === 'scanned' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'
//             }`}
//             onClick={() => setActiveTab('scanned')}
//           >
//             Scanned <span className="ml-1 bg-gray-100 text-gray-600 px-2 py-1 rounded-full text-xs">{scannedProducts.length}</span>
...
//             ))
//           ) : (
//             <div className="text-center py-8 text-gray-500">All items have been picked</div>
//           )
//         ) : (
//           scannedProducts.length > 0 ? (
//             scannedProducts.map(product => (
...
//             ))
//           ) : (
//             <div className="text-center py-8 text-gray-500">No items scanned yet</div>
//           )
//         )}
//       </div>
//     </div>
...
export const SKUInputPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadSKUItems = async () => {
      try {
        const shelfItems = await fetchShelvesInPicklist(id);
        const matchedShelf = shelfItems.find(
          (shelf: any) => shelf.shelfCode === shelfCode
        );
        const skuItems = matchedShelf?.skuInputProducts || [];

        const transformedProducts = skuItems.map((item: any, index: number) => ({
          id: item.id || `sku-${index}`,
  ...
  const handleBack = () => setLocation(`/shelf-selection/${id}`);
  const handleClose = () => setLocation(`/picklist/${id}`);

  const handleSKUInput = () => {
    if (skuInput.trim()) {
      const updated = products.map((p) =>
        p.sku.includes(skuInput.trim()) && p.pickedQuantity < p.quantity
          ? { ...p, pickedQuantity: p.pickedQuantity + 1 }
          : p
      );
      setProducts(updated);
      setSkuInput('');
  ...
  const handlePickItem = (productId: string) => {
    const updated = products.map((p) =>
      p.id === productId && p.pickedQuantity < p.quantity
        ? { ...p, pickedQuantity: p.pickedQuantity + 1 }
        : p
    );
    setProducts(updated);
  ...
  const handleUnpickItem = (productId: string) => {
    const updated = products.map((p) =>
      p.id === productId && p.pickedQuantity > 0
        ? { ...p, pickedQuantity: p.pickedQuantity - 1 }
        : p
    );
    setProducts(updated);
  ...
  const generateShoeImage = (color: string) => {
    ...
    return `data:image/svg+xml,${encodeURIComponent(`
      <svg viewBox="0 0 80 40" xmlns="http://www.w3.org/2000/svg">
        <path d="M10 25 Q15 15 35 18 Q55 20 70 25 Q65 35 45 32 Q25 30 10 25 Z" fill="${fill}" stroke="#374151" stroke-width="1"/>
        <path d="M15 22 Q25 20 45 22 Q55 23 65 25" stroke="white" stroke-width="1.5" fill="none"/>
        <circle cx="20" cy="38" r="1.5" fill="white" opacity="0.6"/>
        <text x="32" y="18" text-anchor="middle" fill="#64748b" font-size="6" font-family="Arial, sans-serif" font-weight="bold">NIKE</text>
      </svg>
  ...
  const ProductCard: React.FC<{ product: Product; isPending: boolean }> = ({
    product,
    isPending,
  }) => {
    const pendingQty = product.quantity - product.pickedQuantity;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-4">
        {loading ? (
          ...
          pendingProducts.length > 0 ? (
            pendingProducts.map((product) => (
              <ProductCard key={product.id} product={product} isPending />
            ))
          ) : (
            <div className="text-center py-8 text-gray-500">
              All items have been picked
