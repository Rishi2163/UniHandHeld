...
export const B2BPackingPage: React.FC = () => {
  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    if (filters.quantity) {
      ...
      filtered = filtered.filter(
        (item) =>
          item.pendingQuantity >= min && item.pendingQuantity <= max
      );
    }
    if (filters.fulfillmentTAT) {
      filtered = filtered.filter((item) =>
        item.fulfillmentTAT.includes(filters.fulfillmentTAT)
      );
    }
  ...
  }, [picklistItems, picklistCode, filters, sortOrder]);
  ...
  return (
    <div className="bg-white min-h-screen w-full flex">
      ...
      <div className="bg-white-100 w-full max-w-md mx-auto min-h-screen relative lg:max-w-none lg:flex-1 lg:ml-0">
        {/* Header */}
        <header className="flex flex-col w-full items-start gap-4 sticky top-0 bg-transparent z-10">
          <div className="flex h-12 items-center relative self-stretch w-full bg-white-100 border-b border-[#e0e0e0]">
            <button
              className="relative w-12 h-12 flex items-center justify-center hover:bg-gray-100 transition-colors duration-200 lg:hidden"
              onClick={handleMenuClick}
            >
            ...
            <div className="flex items-center gap-8 relative flex-1 grow">
              <h1 className="text-xl font-bold text-text-elementsprimary">PICKLISTS</h1>
            </div>
        ...
        {/* Filter Section */}
        <div className="absolute top-12 left-0 right-0 p-4 bg-white-100 border-b lg:px-8">
          <div className="flex items-center gap-3">
            <Button
              variant="outline"
              size="sm"
        ...
        <div className="absolute top-[120px] left-0 right-0 bottom-0 overflow-y-auto">
          <div className="p-4">
            {loading ? (
              <div className="text-center text-gray-500">Loading picklists...</div>
            ) : error ? (
              <div className="text-center text-red-500">{error}</div>
            ) : filteredAndSortedItems.length === 0 ? (
              <div className="text-center text-gray-500">No picklists found.</div>
            ) : (
              <div className="grid grid-cols-1 gap-4">
                {filteredAndSortedItems.map((item) => (
                  <Card
                  ...
                  >
                    <CardContent className="p-4">
                      <div className="mb-3">
                        <h3 className="font-medium text-text-elementsprimary">{item.picklistCode}</h3>
                      </div>
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div>
                          <span className="text-gray-500">Pending Quantity</span>
                          <span className="block font-medium">{item.pendingQuantity}</span>
                        </div>
                        <div>
                          <span className="text-gray-500">Channel</span>
                          <span className="block font-medium">{item.channel}</span>
                        </div>
                        <div>
                          <span className="text-gray-500">Pending Section</span>
                          <span className="block font-medium">{item.pendingSection}</span>
                        </div>
                        <div>
